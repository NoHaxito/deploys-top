...
>(({ className, ...props }, ref) => (
	<Primitive.Content
		ref={ref}
		className={cn(
			"absolute inset-x-0 top-0 data-[motion=from-end]:animate-enterFromRight data-[motion=from-start]:animate-enterFromLeft data-[motion=to-end]:animate-exitToRight data-[motion=to-start]:animate-exitToLeft",
			className,
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<div ref={ref} className="flex w-full justify-center">
		<Primitive.Viewport
			{...props}
			className={cn(
				"relative h-(--radix-navigation-menu-viewport-height) w-full origin-[top_center] overflow-hidden text-popover-foreground transition-[width,height] duration-300 data-[state=closed]:animate-nav-menu-out data-[state=open]:animate-nav-menu-in",
				className,
			)}
		/>
	</div>
